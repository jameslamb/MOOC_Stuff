...
c(unique_evtype1, unique_evtype2, unique_evtype3)
...
unique_evtype4 <- length(unique(data1$EVTYPE2))

## Condense stuff. Change all strings containing a word to that word
...
rationalize <- function(x, strings) {
    ...
    for (i in 1:length(strings)){
        ...
        indx <- grep(strings[i], x)  ## get an index of locations containing this string
        x[indx] <- strings[i]
    }
...
data1$EVTYPE2 <- rationalize(x=data1$EVTYPE2, strings=c("ASTRONOMICAL LOW TIDE", "AVALANCE", "BLIZZARD", "COASTAL FLOOD", "COLD/WIND CHILL", "DEBRIS FLOW", "DENSE FOD", "DENSE SMOKE", "DROUGHT", "DUST DEVEIL", "DUST STROM", "DROUGHT", "DUST DEVIL", "DUST STORM", "EXCESSIVE HEAT", "EXTREME COLD/WIND CHILL", "FLASH FLOOD", "FLOOD", "FREEZING FOG", "FROST/FREEZE", "FUNNEL CLOUD", "HAIL", "HEAT", "HEAVY RAIN", "HEAVY SNOW", "HIGH SURF", "HIGH WIND", "HURRICANE/TYPHOON", "ICE STORM", "LAKESHORE FLOOD", "LAKE-EFFECT SNOW", "LIGHTNING", "MARINE HAIL", "MARINE HIGH WIND", "MARINE STRONG WIND", "MARINE THUNDERSTORM WIND", "RIP CURRENT", "SEICHE", "SLEET", "'STORM TIDE", "STRONG WIND", "THUNDERSTORM WIND", "TORNADO", "TROPICAL DEPRESSION", "TROPICAL STORM", "TSUNAMI", "VOLCANIC ASH", "WATERSPROUT", "WILDFIRE", "WINTER STORM", "WINTER WEATHER", "SUMMARY"))
...
rationalize2 <- function(x,lookup, replacement) {
    ...
    if (class(lookup)!="character") stop('Error: condense_it expected find to be a character string. For example, try something like chars=c("a","b","c")')
    
     if (class(replacement)!="character") stop('Error: condense_it expected find to be a character string. For example, try something like chars=c("a","b","c")')
    
    ## coerce chars to a list                                    
    strings <- str_split(strings, " ", n= Inf) 
    
    ## remove hatever characters you fed to the function
    for (i in 1:length(strings)){
        
        indx <- grep(strings[i], x)  ## get an index of locations containing this string
        x[indx] <- gsub(lookup[i],replacement[i],x)
    }
...
data1$EVTYPE2 <- rationalize2(x=DATA1$EVTYPE2, lookup = c("HURRICANE", "TYPHOON", "VOLCAN"), replacement = c("HURRICANE/TYPHOON", "HURRICANE/TYPHOON", "VOLCANIC ASH"))
